...
function AppWordmark({
  ...
  sopkClassName: string;
  nutritionClassName: string;
}) {
  return (
    <>
      <span className={sopkClassName}>SOPK </span>
      <span className={nutritionClassName}>Nutrition</span>
    </>
  );
}
...
export function BrandLogo({
  ...
  className?: string;
  /** Affiche les mots-clés SOPK (sinon tagline générique). */
  sopkFocus?: boolean;
}) {
  if (variant === "hero") {
    return (
      <div className={cx("flex flex-col items-center text-center", className)}>
        ...
        <p className="text-4xl font-black tracking-[-0.03em] text-brand-900 md:text-6xl md:leading-[1.06]">
          <AppWordmark sopkClassName="font-semibold text-brand-600" nutritionClassName="text-brand-900" />
        </p>
        <div
        ...
        <div className="mt-5 w-full max-w-lg rounded-2xl border border-brand-200/90 bg-gradient-to-b from-brand-50/80 to-white px-5 py-4 shadow-card ring-1 ring-brand-100/50 md:mt-6 md:px-8 md:py-5">
          {sopkFocus ? (
            ...
            <p className="text-body font-medium text-ink md:text-sm">
              Régime personnalisé selon votre profil, votre morphologie et votre objectif.
            </p>
          )}
        </div>
      </div>
    ...
    return (
      <div className={cx("flex min-w-0 items-center gap-2.5", className)}>
        ...
        <div className="min-w-0">
          ...
          <p className="text-eyebrow mt-0.5 truncate text-brand-500/90">{APP_TAGLINE}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={cx("flex shrink-0 select-none flex-col items-center text-center", className)} aria-hidden="true">
        ...
        <p className="text-lg font-black leading-tight tracking-tight sm:text-xl">
          <AppWordmark sopkClassName="font-medium text-brand-200" nutritionClassName="text-white" />
        </p>
      ...
      <div className={cx("flex min-w-0 flex-col items-center text-center", className)}>
        ...
        <div className="mt-2 w-full max-w-sm rounded-xl border border-brand-200 bg-white/85 px-3 py-2 shadow-card sm:px-4 sm:py-2.5">
          {sopkFocus ? (
            ...
            <p className="text-caption font-medium leading-snug text-ink text-balance sm:text-[11px]">
              Programme nutrition adapté à votre profil et votre objectif.
            </p>
          )}
        </div>
      </div>
    ...
    return (
      <div className={cx("flex min-w-0 items-center gap-2.5", className)}>
        ...
        <div className="min-w-0 text-left">
          ...
          <p className="text-[10px] font-medium text-brand-500/90">{APP_TAGLINE}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={cx("flex flex-col items-center text-center", className)}>
      <p className="text-2xl font-black tracking-tight text-brand-900">
        <AppWordmark sopkClassName="font-semibold text-ink-muted" nutritionClassName="text-brand-800" />
      </p>
